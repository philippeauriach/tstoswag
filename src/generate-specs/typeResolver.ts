...
import { getDecorators, getNodeFirstDecoratorValue, isDecorator } from './decoratorUtils'
...
import { getJSDocComment, getJSDocComments, getJSDocTagNames, isExistJSDocTag } from './jsDocUtils'
import { safeFromJson } from './jsonUtils'
import { MetadataGenerator } from './metadataGenerator'
import { getPropertyValidators } from './validatorUtils'
...
export class TypeResolver {
  ...
  public resolve(): Tsoa.Type {
    ...
    if (ts.isTypeLiteralNode(this.typeNode)) {
      ...
      if (indexMember) {
        ...
        additionalType = new TypeResolver(
    ...
    if (ts.isMappedTypeNode(this.typeNode) && this.referencer) {
      ...
      const getDeclaration = (prop: ts.Symbol) =>
        prop.declarations && (prop.declarations[0] as ts.Declaration | undefined)
      ...
      const properties: Tsoa.Property[] = type
        ...
        .map((property) => {
          ...
          if (declaration && ts.isPropertySignature(declaration)) {
            return {
              ...this.propertyFromSignature(declaration, mappedTypeNode.questionToken),
              name: property.getName(),
            }
          } else if (declaration && (ts.isPropertyDeclaration(declaration) || ts.isParameter(declaration))) {
            return {
              ...this.propertyFromDeclaration(declaration, mappedTypeNode.questionToken),
              name: property.getName(),
            }
          }
    ...
    if (ts.isConditionalTypeNode(this.typeNode) && this.referencer && ts.isTypeReferenceNode(this.referencer)) {
      ...
      if (type.aliasSymbol) {
        let declaration = type.aliasSymbol.declarations?.[0] as
          | ts.TypeAliasDeclaration
          | ts.EnumDeclaration
          | ts.DeclarationStatement
        if (declaration.name) {
          declaration = this.getModelTypeDeclaration(declaration.name as ts.EntityName) as
            | ts.TypeAliasDeclaration
            | ts.EnumDeclaration
            | ts.DeclarationStatement
        }
        ...
        const name = this.getRefTypeName(this.referencer.getText())
        return this.handleCachingAndCircularReferences(name, () =>
          this.getModelReference(declaration, this.current.typeChecker.typeToString(type)),
        )
      } else {
        try {
          return new TypeResolver(
            this.current.typeChecker.typeToTypeNode(type, undefined, ts.NodeBuilderFlags.NoTruncation)!,
            this.current,
            this.typeNode,
            this.context,
            this.referencer,
    ...
    if (ts.isTypeOperatorNode(this.typeNode) && this.typeNode.operator === ts.SyntaxKind.KeyOfKeyword) {
      ...
      if (type.isUnion()) {
        const literals = type.types
          ...
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore
          .reduce((acc, t: ts.LiteralType) => [...acc, t.value.toString()], [])
        ...
        if (type.types.find((t) => !t.isLiteral()) !== undefined) {
          ...
          console.warn(
            new GenerateMetaDataWarning(`Skipped non-literal type(s) ${problems.join(', ')}`, this.typeNode).toString(),
          )
        }
        ...
        try {
          ...
          const indexedTypeName = this.current.typeChecker.typeToString(
            this.current.typeChecker.getTypeFromTypeNode(this.typeNode.type),
          )
    ...
    if (
      ...
      if (type === undefined) {
        throw new GenerateMetadataError(
          `Could not determine ${
            numberIndexType ? 'number' : 'string'
          } index on ${this.current.typeChecker.typeToString(objectType)}`,
          this.typeNode,
        )
      }
      return new TypeResolver(
    ...
    if (
      ...
      const hasType = (node: ts.Node | undefined): node is ts.HasType =>
        node !== undefined && Object.prototype.hasOwnProperty.call(node, 'type')
    ...
    if (
      ...
      const isSameTypeQuery =
        ts.isTypeQueryNode(objectType) &&
        ts.isTypeQueryNode(indexType) &&
        objectType.exprName.getText() === indexType.exprName.getText()
      const isSameTypeReference =
        ts.isTypeReferenceNode(objectType) &&
        ts.isTypeReferenceNode(indexType) &&
        objectType.typeName.getText() === indexType.typeName.getText()
      if (isSameTypeQuery || isSameTypeReference) {
        ...
        const node = this.current.typeChecker.typeToTypeNode(
          type,
          undefined,
      ...
      if (type.isUnion() && type.types.every((unionElementType) => unionElementType.isStringLiteral())) {
        ...
        throw new GenerateMetadataError(
          `Could not the type of ${this.current.typeChecker.typeToString(
            this.current.typeChecker.getTypeFromTypeNode(this.typeNode),
            this.typeNode,
          )}`,
          this.typeNode,
        )
  ...
  private hasFlag(type: ts.Symbol | ts.Declaration, flag: any) {
  ...
  private getEnumerateType(typeName: ts.EntityName): Tsoa.RefEnumType | undefined {
    ...
    // resolve value
    let declaredType = (this.current.typeChecker.getDeclaredTypeOfSymbol(symbol)?.symbol || symbol) as ts.Symbol & {
      parent?: ts.Symbol
    }

    // if we are a EnumMember, return parent instead (this happens if a enum has only one entry, not quite sure why though...)
    if (
      this.hasFlag(declaredType, ts.SymbolFlags.EnumMember) &&
      declaredType.parent?.valueDeclaration?.kind === ts.SyntaxKind.EnumDeclaration
    ) {
    ...
    const enums = enumDeclaration.members
      .map((e) => this.current.typeChecker.getConstantValue(e))
  ...
  private getModelReference(modelType: ts.InterfaceDeclaration | ts.ClassDeclaration, name: string) {
    ...
    const deprecated =
      isExistJSDocTag(modelType, (tag) => tag.tagName.text === 'deprecated') ||
      isDecorator(modelType, (identifier) => identifier.text === 'Deprecated')
    ...
    if (
      ...
      (ts.isMethodDeclaration(toJSON.valueDeclaration) || ts.isMethodSignature(toJSON.valueDeclaration))
    ) {
      ...
      if (!nodeType) {
        ...
        nodeType = this.current.typeChecker.typeToTypeNode(
          implicitType,
          undefined,
          ts.NodeBuilderFlags.NoTruncation,
        ) as ts.TypeNode
  ...
  ): ResolvesToPrimitive | DoesNotResolveToPrimitive => {
  ...
  private handleCachingAndCircularReferences(
    name: string,
    declarationResolver: () => Tsoa.ReferenceType,
  ): Tsoa.ReferenceType {
    try {
      const existingType = localReferenceTypeCache[name]
      if (existingType) {
  ...
  private getSymbolAtLocation(type: ts.Node) {
    ...
    return (
      (symbol && this.hasFlag(symbol, ts.SymbolFlags.Alias) && this.current.typeChecker.getAliasedSymbol(symbol)) ||
      symbol
    )
  }
  ...
  private getModelProperties(
    ...
    if (classConstructor && classConstructor.parameters) {
      const constructorProperties = classConstructor.parameters.filter((parameter) =>
        this.isAccessibleParameter(parameter),
      )
  ...
  private getNodeDescription(
    ...
    /**
     * Workaround for what seems like a bug in the compiler
