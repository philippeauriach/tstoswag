...
import { getDecorators, getNodeFirstDecoratorName, getNodeFirstDecoratorValue, isDecorator } from './decoratorUtils'
import { GenerateMetadataError } from './exceptions'
import { getHeaderType } from './headerTypeHelpers'
import { getInitializerValue } from './initializer-value'
import { commentToString, getJSDocTags, isExistJSDocTag } from './jsDocUtils'
import { MetadataGenerator } from './metadataGenerator'
import { TypeResolver } from './typeResolver'
import { getParameterValidators } from './validatorUtils'
...
export class ParameterGenerator {
  ...
  public Generate(): Tsoa.Parameter[] {
    const decoratorName = getNodeFirstDecoratorName(this.parameter, (identifier) =>
  ...
  private getResParameters(parameter: ts.ParameterDeclaration): Tsoa.ResParameter[] {
    const parameterName = (parameter.name as ts.Identifier).text
    const decorator =
      getNodeFirstDecoratorValue(this.parameter, this.current.typeChecker, (ident) => ident.text === 'Res') ||
      parameterName
    if (!decorator) {
    ...
    return statusArgumentTypes.map((statusArgumentType) => {
      if (!isNumberLiteralType(statusArgumentType)) {
        throw new GenerateMetadataError(
          '@Res() requires the type to be TsoaResponse<HTTPStatusCode, ResBody>',
          parameter,
        )
      }
  ...
  private getProducesFromResHeaders(headers: Tsoa.HeaderType): string[] | undefined {
    ...
    const [contentTypeProp] = (properties || []).filter(
      (p) => p.name.toLowerCase() === 'content-type' && p.type.dataType === 'enum',
    )
    if (contentTypeProp) {
  ...
  private getBodyPropParameter(parameter: ts.ParameterDeclaration): Tsoa.Parameter {
    ...
    return {
      ...
      in: 'body-prop',
      name:
        getNodeFirstDecoratorValue(this.parameter, this.current.typeChecker, (ident) => ident.text === 'BodyProp') ||
        parameterName,
      parameterName,
      required: !parameter.questionToken && !parameter.initializer,
      type,
  ...
  private getHeaderParameter(parameter: ts.ParameterDeclaration): Tsoa.Parameter {
    ...
    return {
      ...
      in: 'header',
      name:
        getNodeFirstDecoratorValue(this.parameter, this.current.typeChecker, (ident) => ident.text === 'Header') ||
        parameterName,
      parameterName,
      required: !parameter.questionToken && !parameter.initializer,
      type,
    ...
    if (!this.supportPathDataType(elementType)) {
      throw new GenerateMetadataError(
        `Parameter '${parameterName}:${
          type.dataType
        }' can't be passed as an uploaded file(s) parameter in '${this.method.toUpperCase()}'.`,
        parameter,
      )
    }
  ...
  private getFormFieldParameter(parameter: ts.ParameterDeclaration): Tsoa.Parameter {
    ...
    if (!this.supportPathDataType(type)) {
      throw new GenerateMetadataError(
        `Parameter '${parameterName}:${
          type.dataType
        }' can't be passed as form field parameter in '${this.method.toUpperCase()}'.`,
        parameter,
      )
    }
    ...
    return {
      ...
      in: 'formData',
      name:
        getNodeFirstDecoratorValue(this.parameter, this.current.typeChecker, (ident) => ident.text === 'FormField') ??
        parameterName,
      required: !parameter.questionToken && !parameter.initializer,
      type,
      parameterName,
  ...
  private getQueriesParameters(parameter: ts.ParameterDeclaration): Tsoa.Parameter {
    ...
    if (type.dataType !== 'refObject' && type.dataType !== 'nestedObjectLiteral') {
      throw new GenerateMetadataError(
        `@Queries('${parameterName}') only support 'refObject' or 'nestedObjectLiteral' types. If you want only one query parameter, please use the '@Query' decorator.`,
      )
    }
  ...
  private validateQueriesProperties(property: Tsoa.Property, parentName: string) {
    if (property.type.dataType === 'array') {
      ...
      if (!this.supportPathDataType(arrayType.elementType)) {
        throw new GenerateMetadataError(
          `@Queries('${parentName}') property '${property.name}' can't support array '${arrayType.elementType.dataType}' type.`,
        )
      }
    } else if (!this.supportPathDataType(property.type)) {
      throw new GenerateMetadataError(
        `@Queries('${parentName}') nested property '${property.name}' Can't support '${property.type.dataType}' type.`,
      )
    }
  }
  ...
  private getQueryParameters(parameter: ts.ParameterDeclaration): Tsoa.Parameter[] {
    ...
    const commonProperties = {
      ...
      in: 'query' as const,
      name:
        getNodeFirstDecoratorValue(this.parameter, this.current.typeChecker, (ident) => ident.text === 'Query') ||
        parameterName,
      parameterName,
      required: !parameter.questionToken && !parameter.initializer,
      validators: getParameterValidators(this.parameter, parameterName),
    ...
    if (this.getQueryParameterIsHidden(parameter)) {
      if (commonProperties.required) {
        throw new GenerateMetadataError(
          `@Query('${parameterName}') Can't support @Hidden because it is required (does not allow undefined and does not have a default value).`,
        )
      }
      return []
    }
  ...
  private getPathParameter(parameter: ts.ParameterDeclaration): Tsoa.Parameter {
    ...
    const pathName = String(
      getNodeFirstDecoratorValue(this.parameter, this.current.typeChecker, (ident) => ident.text === 'Path') ||
        parameterName,
    )
  ...
  private getParameterDeprecation(node: ts.ParameterDeclaration) {
    return (
      isExistJSDocTag(node, (tag) => tag.tagName.text === 'deprecated') ||
      isDecorator(node, (identifier) => identifier.text === 'Deprecated')
    )
  }
  ...
  private getParameterExample(node: ts.ParameterDeclaration, parameterName: string) {
    ...
    const examples = getJSDocTags(node.parent, (tag) => {
      ...
      const isExample =
        (tag.tagName.text === 'example' || tag.tagName.escapedText === 'example') &&
        !!tag.comment &&
        comment?.startsWith(parameterName)
      ...
      (commentToString(tag.comment) || '')
        .replace(`${commentToString(tag.comment)?.split(' ')[0] || ''}`, '')
        .replace(/\r/g, ''),
    )
  ...
  private supportPathDataType(parameterType: Tsoa.Type): any {
    const supportedPathDataTypes: Tsoa.TypeStringLiteral[] = [
      'string',
      'integer',
      'long',
      'float',
