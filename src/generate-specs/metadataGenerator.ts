...
export class MetadataGenerator {
  ...
  constructor(
    ...
    this.program = controllers
      ...
      : createProgram([entryFile], compilerOptions || {})
    ...
    controllers.forEach((controller) => {
      controller.methods.forEach((method) => {
        const signature = method.path
          ? `@${method.method}(${controller.path}/${method.path})`
          : `@${method.method}(${controller.path})`
  ...
  private checkForPathParamSignatureDuplicates = (controllers: Tsoa.Controller[]) => {
    ...
    function addCollision(
      ...
      let existingCollision = collisions.find(
        (collision) => collision.type === type && collision.method === method && collision.controller === controller,
      )
    ...
    collisions.forEach((collision) => {
      ...
      message += ` [ method ${collision.method.method.toUpperCase()} ${collision.method.name} route: ${
        collision.method.path
      } ] collides with `
